...
export function CronotypeProfileSkeleton() {
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-1 items-center gap-6 sm:grid-cols-[auto_1fr] sm:gap-10">
          ...
          <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3 sm:mt-4 sm:flex sm:flex-wrap sm:items-end sm:gap-x-8">
            {[24, 28, 24, 20].map((w, i) => (
              <div key={i} className="flex flex-col gap-1.5">
                <div className="skeleton h-2.5" style={{ width: 44 }} />
                <div className="skeleton h-6 sm:h-7" style={{ width: w + 24 }} />
              </div>
            ))}
          </div>
        </div>
...
function HaloSkeleton() {
  return (
    <div className="relative">
      ...
      <div className="bg-paper dark:bg-ink absolute inset-0 m-auto h-[64px] w-[64px] rounded-full sm:h-[100px] sm:w-[100px]" />
    </div>
  );
}
